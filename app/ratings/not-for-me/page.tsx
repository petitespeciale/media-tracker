...
export default function NotForMePage() {
    ...
    const notForMeItems = getWatched().filter(item => item.rating === 1);
        ...
        <main className="min-h-screen p-4 pb-24 md:p-8">
            <div className="mx-auto max-w-5xl space-y-8">
                <header className="flex items-center gap-4 py-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold">😐 Not For Me</h1>
                        <p className="text-sm text-muted-foreground">{notForMeItems.length} items</p>
                    </div>
                </header>

                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                    {notForMeItems.map((item) => (
                        <Link
                            key={item.id}
                ...
                {notForMeItems.length === 0 && (
                    <div className="text-center text-muted-foreground py-20">
                        You haven't rated anything as "Not For Me" yet.
                    </div>
                )}
            </div>
        </main>
