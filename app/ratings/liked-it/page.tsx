...
export default function LikedItPage() {
    const { getWatched } = useStore();
    const likedItems = getWatched().filter(item => item.rating === 2);
        ...
        <main className="min-h-screen p-4 pb-24 md:p-8">
            <div className="mx-auto max-w-5xl space-y-8">
                <header className="flex items-center gap-4 py-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold">😊 Liked It</h1>
                        <p className="text-sm text-muted-foreground">{likedItems.length} items</p>
                    </div>
                </header>
                ...
                {likedItems.length === 0 && (
                    <div className="text-center text-muted-foreground py-20">
                        You haven't rated anything as "Liked It" yet.
                    </div>
                )}
            </div>
        </main>
